package app.example.Animation.ObjectsInMotion;

import java.util.Random;

import app.example.DragAndDrop.TableViewCellDraging.GetCellAndColumnIndexWithCellDragedDemo;
...
public class ObjectsInMotionDemo extends Application {
	
  private static Random random = new Random(42);
  private static final int    N_OBJECTS   = 1000;
  private static final int    OBJECT_SIZE = 20;
  private static final Image  KLINGON = new Image(ObjectsInMotionDemo.class.getResourceAsStream("Snow-flake-icon.png"), OBJECT_SIZE, OBJECT_SIZE, true, true);
  
  public ObjectsInMotionDemo() {
		Platform.runLater(() -> {           
	        try {
				start(new Stage());
			} catch (Exception e) {
