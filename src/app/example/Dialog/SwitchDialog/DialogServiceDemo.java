package app.example.Dialog.SwitchDialog;
 ...
 */
public class DialogServiceDemo extends Application {
	...
	public static void main(final String[] args) {
		try {
			Application.launch(DialogServiceDemo.class, args);
	...
	public void start(final Stage primaryStage) throws Exception {
		...
		primaryStage.getScene().getStylesheets().add(
				DialogServiceDemo.class.getResource("dialog.css").toExternalForm());
		primaryStage.show();
	}
	...
	public static DialogService dialog(final Stage parent, final String title,
		...
		final Text header = new Text(headerText);
		header.getStyleClass().add("dialog-title");
		header.setWrappingWidth(width / 1.2d);
		
		final Text messageHeader = new Text();
		messageHeader.getStyleClass().add("dialog-message");
		messageHeader.setWrappingWidth(width / 1.2d);
		...
		final Button submitBtn = new Button(submitLabel);
			...
			this.submitService.stateProperty().addListener(new ChangeListener<State>() {
				...
				public void changed(final ObservableValue<? extends State> observable,
					...
					if (submitService.getException() != null) {
						...
						window.getScene().getRoot().setEffect(colorAdjust);
